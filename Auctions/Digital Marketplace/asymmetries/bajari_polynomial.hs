...
upperBoundBidsFunc lowers uppers =
  let bs = NC.linspace 10000 ((head uppers), (uppers !! 1))
      cdfs = zipWith (\l u -> uniformCDF l u) (drop 1 lowers) (drop 1 uppers)
      negCdfs x = map (\cdf -> 1 - (cdf x)) cdfs
      objective x = (x - (head uppers)) * (foldr1 (\c acc -> acc * c) $ negCdfs x)
...
main = do
  ...
  let (s,p) = GSL.minimize GSL.NMSimplex2 1E-8 100000 (take 13 [1E-2,1E-2..]) objective (take 13 ([l1] ++ [1E-2,1E-2..]))
  let bLow = head s
